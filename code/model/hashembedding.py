...
class LSHHashEmbedding(nn.Module):
    r"""Doc.
    Temp
    """
    def __init__(self,
        ...
        # num_embeddings needs to be a power of 2 since our hashes produce bits
        num_embeddings = int(2**math.ceil(math.log2(num_embeddings)))
        self.num_embeddings = num_embeddings

        self.embedding_dim = embedding_dim
        self.num_hashes = num_hashes

        # num_buckets also needs to be a power of 2
        num_buckets = int(2**math.ceil(math.log2(num_buckets)))
        ...
        self.importance_weights = nn.Embedding(self.num_embeddings,
                                               self.num_hashes)
        self.shared_embeddings = nn.Embedding(self.num_buckets,
                                              self.embedding_dim,
                                              padding_idx=self.padding_idx)

        self.weighthash = RandomProjectionHashModule(
            last_input_dim_size, int(math.log2(self.num_embeddings)))
        hashes = [
            RandomProjectionHashModule(last_input_dim_size,
                                       int(math.log2(self.num_buckets)))
            for _ in range(self.num_hashes)
        ]
        self.hashes = nn.ModuleList(hashes)
    ...
    def reset_parameters(self,
                         init_shared=lambda x: normal_(x, std=0.1),
                         init_importance=lambda x: normal_(x, std=0.0005)):
        ...
        init_shared(self.shared_embeddings.weight)
        init_importance(self.importance_weights.weight)

        if self.padding_idx is not None:
            # Unfortunately has to set weight to 0 even when paddingIdx = 0
            self.shared_embeddings.weight = set_constant_row(
                self.shared_embeddings.weight)
            self.importance_weights.weight = set_constant_row(
                self.importance_weights.weight)
    ...
    def forward(self, input):
        idx_importance_weights = self.weighthash(input)

        idx_shared_embeddings = torch.stack([h(input) for h in self.hashes],
                                            dim=-1)

        # print(idx_shared_embeddings, idx_shared_embeddings.size())
        ...
        # print(importance_weight, importance_weight.size())
...
class HashEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        self.embedding_dim = embedding_dim
        self.num_hashes = num_hashes
        defaultNBuckets = (num_embeddings *
                           self.num_hashes) // (self.embedding_dim)
    ...
    def forward(self, input):
        ...
        idx_shared_embeddings = torch.stack(
            [h(input).masked_fill_(input == 0, 0) for h in self.hashes],
            dim=-1)
        ...
        importance_weight = importance_weight.unsqueeze(-2)
        # print(importance_weight, importance_weight.size())
        # print(importance_weight.size(), shared_embedding.size())
        word_embedding = self.aggregate(importance_weight * shared_embedding)
        if self.append_weight:
            # concateates the vector with the weights
            word_embedding = torch.cat(
